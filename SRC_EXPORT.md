...
C:\Users\Endri Azizi\progetti-dev\ionic-conference-app
├── .angular
├── .editorconfig
├── .eslintrc.json
├── .firebaserc
...
<ion-content fullscreen class="infiorata-page">
  <!-- Browser/PWA: iframe con URL “sanitized” e SANDBOX per bloccare uscite -->
  <iframe
    *ngIf="isBrowser"
    class="ext-frame"
    [src]="safeUrl"
  ...
  <!-- App mobile: qui potresti aprire un in-app browser/ThemeableBrowser -->
  <p *ngIf="!isBrowser" class="loading-msg">
...
:root{
  ...
  --cta-video-lift: -30px;  /* ↓ più piccolo = bottoni più vicini al footer */
...
.info-kiosk{
  text-align:center;
  background:#f4f4f4;
  padding:clamp(10px,1.6vh,18px) 10px; /* 🔧 leggermente meno padding */
  color:#222; font-weight:600;
  border-radius:8px;
  margin: 6px 12px 0; /* 🔧 tolto il margin-bottom che creava “buco” */
}
...
.slide{
  ...
  padding:1px 12px 8px; /* 🔧 meno padding top */
  gap:10px; /* 🔧 leggermente ridotto */
  text-align:center;
}
...
.info-card{
  ...
  padding:16px 16px 14px; /* 🔧 un filo meno alta */
...
/* CTA vicina al footer */
/* DOPO (ancorato davvero vicino al footer) */
.video-cta{
  bottom: calc(
    max(8px, env(safe-area-inset-bottom)) +  /* safe area iOS se c'è */
    var(--cta-video-lift)                    /* puoi metterlo anche negativo tipo -4px */
  );
  left: 50%;
...
/* nascondi UI quando la slide video non è attiva */
...
/* =========================
   OVERRIDE IPHONE 14 PRO MAX (430×932, portrait)
   ========================= */
...
/* Container che occupa tutta la toolbar */
.toolbar-container {
  position: absolute;
  inset: 0;
  display: flex;
...
/* Immagine ridimensionata senza crop */
